Matchmakers shift the monopoly of choosing which bundle is included to someone with the users’ best interests in mind. We call this actor a Matchmaker. You can learn more about the theory of matchmakers here. While a trusted actor is necessary today, in the future the forwarding role may be decentralized.

One interesting notion of MemSwap is that many matchmakers can exist. One important benefit of this is that

Matchmakers are responsible for incentivizing solvers to offer better prices for their intent solutions. To be eligible for the benefits the matchmaker offers, a user or app should restrict the filler of their intents to the matchmaker's address. Using the built-in authorization mechanism of Memswap, the matchmaker will then only authorize for filling the solver(s) which offer(s) the best price to the user. T

You can check out the reference Reservoir matchmaker implementation here.

The reference implementation uses the signature authorization mechanism as follows:
